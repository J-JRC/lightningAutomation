...
public class RestAssuredDriver {
	...
	public RestAssuredDriver(IAuthentication auth) {
		this.auth = auth;
	}

	public RestAssuredDriver() {
	}
	...
	/**
	 * This method is for login at openAM and set cookies It will also get
	 * access_token if API gateway URL is given in textNG context
	 * 
	 * @param username:
	 *            String
	 * @param password:
	...
	private RequestSpecification generateCommonReqSpec(File file, BaseRestParameter params) {
		if(this.auth != null) {
			return this.auth.getAuthenticationReqSpec().headers(this.auth.getAuthenticationHeader()).queryParams(params.getQuery()).multiPart(file)
					.contentType("multipart/form-data");
		}
		return given().queryParams(params.getQuery()).multiPart(file)
				.contentType("multipart/form-data");
	}
	...
	private RequestSpecification generateCommonReqSpec() {
		if(this.auth != null) {
			return this.auth.getAuthenticationReqSpec().headers(this.auth.getAuthenticationHeader()).contentType(CHARSET_UTF_8).accept(ContentType.JSON)
					.contentType(ContentType.JSON);
		}
		return given().contentType(CHARSET_UTF_8).accept(ContentType.JSON)
				.contentType(ContentType.JSON);
	}
	...
	/**
	 * This method is for setting Request specification with minimal parameter
	 * settings, no content type or accept type are set
	 * 
	 * @param params:
	 *            BaseRestParameter
	 * @return RequestSpecification
	...
	private RequestSpecification generateReqSpecWithNoPayload(BaseRestParameter params) {
		...
		if(this.auth != null)
			return this.auth.getAuthenticationReqSpec().headers(this.auth.getAuthenticationHeader()).headers(params.getHeader()).queryParams(params.getQuery());
	...
	private String generateURL(String uri) {
		String finalURI = "";
		finalURI = ControlCenter.getInstance().getParameter(API_SERVER) + uri;
	...
	/**
	 * This method is to perform POST with the specified uri. Input JsonRequest will
	 * be treated as request body
	 * 
	 * @param uri:
	 *            String
	 * @param params:
	...
	/**
	 * This method is to perform POST with a json request body with the specified
	...
	/**
	 * This method is to perform POST with a json request body with the specified
	 * uri. Input JsonRequest will be treated as request body, specification will be
	 * rest assured RequestSpecification object in case the provided method does not
	 * satisfy your need.
	 * 
	 * @param payload:
	 *            Request
	...
	/**
	 * This method is to perform POST with a file as attachment and the specified
	 * URI.
	 * 
	 * @param URI:
	 *            String
	...
	/**
	 * This method is to perform GET with a json request body with the specified
	 * uri. Input JsonRequest will be treated as request body, specification will be
	 * rest assured RequestSpecification object in case the provided method does not
	 * satisfy your need.
	 * 
	...
	public APIResponse put(String uri, BaseRestParameter params, RequestSpecification specification) throws Exception {
		reportingBeforeSend(Operations.PUT,generateURL(uri, params), params, null);
		Response res = RestAssuredUtils.put(generateURL(uri, params), null, generateReqSpecWithNoPayload(params).spec(specification));
		reportingAfterSend(res);
	...
	/**
	 * This method is to perform DELETE with a json request body with the specified
	 * uri. Input JsonRequest will be treated as request body
	 * 
	 * @param uri:
	 *            String
	...
	public APIResponse delete(String uri, BaseRestParameter params, String payload, RequestSpecification specification)
			throws Exception {
		reportingBeforeSend(Operations.DELETE,generateURL(uri, params), params, payload);
		Response res = RestAssuredUtils.delete(generateURL(uri, params), payload, generateReqSpecWithNoPayload(params).spec(specification));
		reportingAfterSend(res);
	...
	/**
	 * This method is to perform DELETE with the specified uri. Specification will
	 * be rest assured RequestSpecification object in case the provided method does
	 * not satisfy your need.
	 *
	 * @param uri:
	 *            String
	 * @param params:
	...
	/**
	 * This method is to perform PATCH with the specified uri. Input JsonRequest
	 * will be treated as request body
	 * 
	 * @param uri:
	 *            String
	 * @param params:
	...
	/**
	 * This method is to perform PATCH with a json request body with the specified
	 * uri. Input JsonRequest will be treated as request body, specification will be
	 * rest assured RequestSpecification object in case the provided method does not
	 * satisfy your need.
	 * 
	...
	public APIResponse patch(String uri, BaseRestParameter params, String payload, RequestSpecification specification)
			throws Exception {
		reportingBeforeSend(Operations.PATCH, generateURL(uri, params), params, payload);
		Response res = RestAssuredUtils.patch(generateURL(uri, params), payload, generateReqSpecWithNoPayload(params).spec(specification));
		reportingAfterSend(res);
