...
public class BaseElement implements IBaseElement {
	...
	public boolean isEnable() {
	    ...
	    /*
	     * some button rely on the css for disable instead of disable attribute
	     * if isEnable is true, we further look for css property cursor.
	     * 
	     */
	...
	public void waitForEnable() throws FrameworkException {
		if(this.element == null) {
			this.element = BasicElementGateway.findElementByPresence(driver, LocatorUtil.determineByType(locatorString.split(":")[1]), TIMEOUT);
	...
	public void waitForDisable() throws FrameworkException {
		if(this.element == null) {
			...
			if(locator instanceof DefaultElementLocator) {
				...
				// we assume that if a element is disabled, it should be visible to be located
	...
	public void waitForNotPresent() throws FrameworkException {
		if(this.element == null) {
			this.element = BasicElementGateway.findElementByNotPresence(driver, LocatorUtil.determineByType(locatorString.split(":")[1]), 1);
		}else {
			if(locator instanceof DefaultElementLocator) {
		...
		try {
			if(target instanceof Proxy){
				WebElement elt = (WebElement)Proxy.getInvocationHandler(target).invoke(target, 
													BaseElement.class.getMethod("getWrappedElement", new Class[]{} ), 
													null);
