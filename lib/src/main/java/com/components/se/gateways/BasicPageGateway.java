...
public class BasicPageGateway {
	 ...
	 */
	public static void waitForFrameToLoadAndSwitch(WebDriver driver, String locator, int timeout) throws FrameworkException {
		try {
			CustomSeleniumGateway.frameToBeAvailableAndSwitchToIt(
					driver, 
					LocatorUtil.determineByType(locator),
					timeout);
		} catch (Exception e) {
	...
	public static void waitForContentToLoad(WebDriver driver, String locator, int timeout) throws FrameworkException {
		...
		try {
			CustomSeleniumGateway.waitUntilElementIsInvisible(
					driver, 
					LocatorUtil.determineByType(locator),
					timeout
			);
