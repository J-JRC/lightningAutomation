...
public class CustomSeleniumGateway {
	...
	/**
	 * Wait for element to be disable
	 * attributes
	 * 
	 * @param driver WebDriver
	 * @param elt WebElement
	...
	public static Boolean waitUntilElementIsInvisible(WebDriver driver, By locator, int timeout) {
		return (new WebDriverWait(driver, Duration.ofSeconds(timeout))).until(invokeElementExpectedConditionByInvisiblility(locator));
	}
	    ...
	    return new ExpectedCondition<Boolean>() {
	      ...
	      public Boolean apply(WebDriver driver) {
	        try {
	        	boolean isEnabled = elt.isEnabled();
        	    /*
        	     * some of our button rely on the css for disable instead of disable attribute
        	     * if isEnable is true, we further look for css property cursor.
        	     */
        	    if(isEnabled && elt.getCssValue("cursor").equalsIgnoreCase("not-allowed")){
        	        isEnabled=false;
