...
public class TestExtentResultListener implements ITestListener, IInvokedMethodListener {
	...
	public void onStart(ITestContext context) {
		//add extent reporter
		try {
			reporter = new ReporterExtent(
					controlCenter.getDirectory(CURRENT_REPORT),
					controlCenter.getDirectory(CURRENT_SC),
		...
		if (method.isConfigurationMethod()) {
			try {
				...
				if(m.getAnnotation(BeforeClass.class) != null) {
					...
					reporter.info("<h6><strong> ========== Start Before Class ["+ m.getName() + "()] ========== </strong></h6>");
				}
				
				if(m.getAnnotation(BeforeMethod.class) != null) {
					reporter.info("<h6><strong> ========== Start Before Method ["+ m.getName() + "()] ========== </strong></h6>");
				}
				...
				if(m.getAnnotation(AfterClass.class) != null) {
					reporter.info("<h6><strong> ========== Start After Class ["+ m.getName() + "()]  ========== </strong></h6>");
				}
