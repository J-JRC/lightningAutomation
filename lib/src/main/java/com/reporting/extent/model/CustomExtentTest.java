...
public class CustomExtentTest {
	...
	//we need to know where the setup steps stops in the class test so that we can just add the cleanup step to the method in the after class later
	private int setupMethodStopIndex;

	public CustomExtentTest(ExtentTest test, int retryCounter, String caseName,String description, CustomExtentTest testAtClass) {
		if(testAtClass == null){ 
			this.test = test ;
