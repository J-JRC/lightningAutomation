...
public class ControlCenter {
	...
	public void initialize(Map<String, String> params) throws FrameworkException {
		...
		try {
			...
			setDirectory(params);
			//Add default logger
			this.addReporter(new ReporterLog4J(params, directories.get(LOG_LOCATION) + File.separator + directories.get(PARTIAL_NAME) + ".log"));
	...
	private void setDirectory(Map<String, String> params) throws IOException {
		...
		try {
			...
			directories.put(REPORT, (params.containsKey(REPORT_LOCATION) ? params.get(REPORT_LOCATION) : root) + "/reports/");
			...
			directories.put(LOG_LOCATION, params.containsKey(LOG_LOCATION) ? params.get(LOG_LOCATION) : directories.get(CURRENT_REPORT));
