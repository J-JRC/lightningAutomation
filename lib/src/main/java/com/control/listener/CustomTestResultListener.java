...
import com.assertion.BaseAssertion;
...
//				ITestNGMethod currentMethod = Arrays.asList(result.getTestContext().getAllTestMethods()).stream().filter(x-> x.getMethodName().equals(m.getName())).collect(Collectors.toList()).get(0);
...
//		}
	...
	/**
	 * If retry is triggered, the test case will be considered as skip, but this
	 * skip test actually count in the total test case. 
	 * For example, 2 test cases are executed, one failed and got retried for 3 times, and at the end the total
	 * run will be 5 test cases but in reality, we only run 2 test cases. This code
	 * will remove the retry from the total run count.
	 */
	@Override
